...
import { DEFAULT_STATIC_IMAGES, STATIC_RESOURCE_FIELDS } from '../../lib/constants/path-transformer.constant.js';
      ...
      for (const key in current) {
        if (Object.hasOwn(current, key)) {
          ...
          if (this.isStaticProperty(key) && typeof value === 'string') {
            ...
            current[key] = `${getServerUri(serverHost, serverPort)}${rootPath}/${value}`;
          }
        }
      }
    }
